...
export default function Page() {
    ...
    return (
        <section className="flex flex-col gap-3">
            <h1 className="text-2xl font-bold text-darkblue">계정 관리</h1>
            {/* 나의 정보 */}
            <div>
              <h3 className="text-xl font-semibold text-slate-800 p-2">나의 정보</h3>
                <div className="bg-slate-100 p-7 rounded-lg text-gray-700">
                    여기에 사용자 정보가 표시됩니다.
                </div>
            </div>
            {/* 사용자 전환 */}
            <div>
              <h3 className="text-xl font-semibold text-slate-800 p-2">사용자 전환</h3>
                <div className="bg-green-100 p-5 rounded-lg flex justify-between items-center gap-5">
                    <span className="text-green-700">여기에 사용자의 계정 권한이 전환됩니다</span>
                    <button 
                        onClick={openModal}
                ...
                <div className="flex items-center justify-center fixed inset-0 backdrop-filter backdrop-blur-sm z-20">
                    <div className="absolute w-70% px-7 py-10 rounded-md shadow-lg bg-blue flex flex-col items-center justify-between">
                        ...
                        <div className="flex flex-col font-semibold text-md gap-3">
                            ...
                            <button 
                                className="bg-green-500 text-white py-2 px-4 rounded-md transition-transform duration-150 hover:bg-green-600">네. 계정 유형을 변경합니다</button>
                        </div>
                    </div>
                </div>
            )}
